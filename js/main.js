...
function applyWindow(fundings, windowRange) {
    if (!windowRange || !windowRange.start || !windowRange.end) {
        return deduplicateFundings(fundings.map((funding) => enrichFunding(funding)));
    }

    const start = windowRange.start;
    const end = windowRange.end;

    const enriched = fundings.map((funding) => enrichFunding(funding, start, end));
    const deduplicated = deduplicateFundings(enriched);
...
function canonicalizeUrlForKey(url) {
    ...
    try {
        ...
        const normalisedPath = parsed.pathname.replace(/\/+/g, '/').replace(/\/+$, '') || '/';
...
function deduplicateFundings(fundings) {
    const seenSources = new Set();
    const seenTitles = new Set();
    const results = [];

    for (const funding of fundings) {
        const sourceKey = canonicalizeUrlForKey(
            funding.scraped_from || funding?.application?.application_url || ''
        );
        const titleValue = (funding.title || '').trim().toLowerCase();
        const orgValue = (funding.organization || '').trim().toLowerCase();
        const hasTitleKey = Boolean(titleValue || orgValue);
        const titleKey = `${titleValue}::${orgValue}`;

        let isDuplicate = false;
        if (sourceKey && seenSources.has(sourceKey)) {
            isDuplicate = true;
        }
        if (!isDuplicate && hasTitleKey && seenTitles.has(titleKey)) {
            isDuplicate = true;
        }
